...
Example: {{name}}`,context:`{
  ...
  name: "Bob",
  role: "admin"
}`}},s=(o,r)=>{let e=document.getElementById(o);if(!(e instanceof r))throw Error(`Missing or wrong-type element: ${o}`);return e},c=s("template",HTMLTextAreaElement),l=s("context",HTMLTextAreaElement),k=s("render",HTMLButtonElement),b=s("output",HTMLPreElement),u=s("log",HTMLPreElement),F=s("call",HTMLPreElement),f=s("example",HTMLSelectElement);function v(o){let r=z[o];if(!r)return;c.value=r.template,l.value=r.context,x()}f.addEventListener("change",()=>v(f.value));function M(o){let r=o.trim();if(!r)return{};let i=Function(`"use strict"; return (${r});`)();if(i===null||typeof i!=="object")throw TypeError("Context must be an object literal.");let t={};for(let[a,n]of Object.entries(i)){if(typeof n!=="string"&&typeof n!=="function")throw TypeError(`Context value "${a}" must be a string or function, got ${typeof n}`);t[a]=n}return t}function d(o,r){let e=document.createElement("span");e.className=r,e.textContent=o+`
`,u.appendChild(e)}function h(o,r){let e=" ".repeat(r);return o.split(`
`).map((i,t)=>t===0?i:e+i).join(`
`)}function S(o){return"`"+o.replace(/\\/g,"\\\\").replace(/`/g,"\\`")+"`"}function R(o){let r=o.trim();return r?r:"{}"}function p(){let o=S(c.value),r=R(l.value);F.textContent=`render(
  `+h(o,2)+`,
  `+h(r,2)+`
);`}function x(){p(),b.textContent="",u.textContent="";let o;try{o=M(l.value)}catch(r){d(`Context parse error: ${r instanceof Error?r.message:String(r)}`,"error");return}try{let r=g(c.value,o);b.textContent=r,d(`rendered ${r.length} chars`,"info")}catch(r){if(r instanceof Error)d(`${r.name}: ${r.message}`,"error");else d(String(r),"error")}}k.addEventListener("click",x);c.addEventListener("input",p);l.addEventListener("input",p);c.addEventListener("keydown",w);l.addEventListener("keydown",w);function w(o){if(o.key!=="Tab")return;o.preventDefault();let r=o.currentTarget;if(!(r instanceof HTMLTextAreaElement))return;let{selectionStart:e,selectionEnd:i}=r;r.value=r.value.slice(0,e)+"  "+r.value.slice(i),r.selectionStart=r.selectionEnd=e+2}v("hello");
